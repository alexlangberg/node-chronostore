...
var cs = {
  ...
  'write': function(options) {
    ...
    if (options.log && !cs.logger) {
      var folder = options.log.path || './chronostore-logs';
      ...
      cs.logger.on('rotated', function(filename) {
        var rotationLogger = fs.createWriteStream(
          folder + '/' + 'rotation.log',
          {'flags': 'a'}
        );

        rotationLogger.end(path.basename(filename) + '\n');
      });
    }
